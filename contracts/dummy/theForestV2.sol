...
contract TheRarityForestV2 is ERC721Enumerable {
    ...
    constructor(address _rarityAddr, address _rarityForestAddr, address _xpProxy) ERC721(_rarityAddr) {
        rarityContract = IRarity(_rarityAddr);
        rarityForestContract = ITheRarityForest(_rarityForestAddr);
		rarityXPProxy = IRarityXPProxy(_xpProxy)
    }

    IRarityXPProxy public rarityXPProxy;
    ...
    function levelUp(uint256 tokenId) public {
        ...
        rarityXPProxy.spend_xp(summonerId, xpRequired(current));
        level[tokenId] += 1;
